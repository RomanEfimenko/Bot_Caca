...
path_controll_proces = "python test_bot_hard_for_auto.py";
path_console_script = "python data_arn/auto_runbot_cnsle.py"
...
while working:
    ...
    if (time.time()-read_nowsession_time) > 5: # каждые 2 секунд
        ...
        if time.time()-get_secs(nowsession_file_text_ahahaha) > 20 : # 20 секунд назад последняя запись? рестарт
            ...
            try:
                kill(proc.pid)
                text_fileee = open("data_arn/nowsession.txt", "w");
                text_fileee.write(str(time.time()));
                text_fileee.close();
            except:
