...
woman_gs_bool = True # Если тру сразу выводит список женских фраз

clear = lambda: os.system('cls')

packs = "S:\GitHub\Bot_Caca\Bot_Caca\DayzeeCode\sounds" # директория со звуками # "/storage/emulated/0/Send audio file"
...
client.login(email='lousenpai228@gmail.com', password='15051999')
#client.login(email='efimenko@ucoz-team.net', password='123456789')
#client.login(email='@', password='')
print(f"{bc.blue} Вошёл в аккаунт")

subclient = amino.SubClient(comId='41242618', profile=client.profile) # 156542274 твой ID сообщества что был тут
...
if woman_gs_bool :
    ...
    ind_papka = 0 # Счетчик для "for name_papka in papki"
    papki = ["man","girl","woman_gs"] # Список папок
    for name_papka in papki :
        print(str(ind_papka)+". "+name_papka) # Выводим все папки что в масиве "papki"
        ind_papka = ind_papka + 1 # Увеличиваем индекс
    otvet_papka = input("Введите номер нужной папки: ") # Указываем индекс нужной нам папки
    which_pack = papki[int(otvet_papka)] # Находим название папки по индексу
    print("Вы выбрали папку '"+which_pack+"';\n")
elif woman_gs_bool == False :
    print(f"{bc.yellow} Фразы женским голосом")     
    which_pack = "woman_gs"
else :
    print(f"{bc.yellow} Сейчас, будем выбирать пак. Вывожу список папок")
